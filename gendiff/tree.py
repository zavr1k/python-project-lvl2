...
def add_children(file1: dict, file2: dict) -> list:
    ...
    for key in common_keys:
        if key not in file1:
            result.append({"key": key, "type": ADDED, "value": file2[key]})
        elif key not in file2:
            result.append({"key": key, "type": REMOVED, "value": file1[key]})
        elif file1[key] == file2[key]:
            result.append({"key": key, "type": UNCHANGED, "value": file2[key]})
...
def get_diff(file1: dict, file2: dict) -> dict:
    diff = {"type": ROOT, "children": add_children(file1, file2)}
    return diff
